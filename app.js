...
/* V5.6 HOME HUD live database module */
function updateCommandDeck(){
 const el=document.getElementById("deckEntries");
 if(el) el.textContent=(Array.isArray(comics)?comics.length:0)+" COMICS";
}
setTimeout(updateCommandDeck,4200);
const _refreshV56=refresh;
refresh=async function(){
 const r=await _refreshV56.apply(this,arguments);
 updateCommandDeck();
 return r;
};

/* V5.7 — interaction polish */
(function(){
 let lastView="";
 document.addEventListener("click",e=>{
   const n=e.target.closest("nav button");
   if(n){
     const id=n.dataset?.view||n.textContent.trim();
     if(id!==lastView){
       const t=document.getElementById("tabTransition");
       if(t){t.classList.remove("play");void t.offsetWidth;t.classList.add("play")}
       lastView=id;
     }
   }
 },{passive:true});
 // Native-feeling haptic where supported, intentionally tiny.
 document.addEventListener("click",e=>{
