...
(function(){
 ...
 function cinematicOpen(img){
   ...
   setTimeout(()=>o.classList.remove("play"),620);
 }
 const car=document.getElementById("carousel");
 if(car){
   car.addEventListener("pointermove",e=>{
     if(matchMedia("(pointer:fine)").matches){
       const r=car.getBoundingClientRect(),x=(e.clientX-r.left)/r.width-.5,y=(e.clientY-r.top)/r.height-.5;
       car.style.setProperty("--tiltX",(-y*3)+"deg"); car.style.setProperty("--tiltY",(x*4)+"deg");
     }
   });
   car.addEventListener("pointerleave",()=>{car.style.setProperty("--tiltX","0deg");car.style.setProperty("--tiltY","0deg")});
 }
